...
public class GA {
  ...
  public GA(BigInteger n, int cross, int mut) {
    ...
    if (length % 2 == 0) {
        chromLength = length/2 - 2 + 0; //subtract 2 to get chromosome length that is divided by 6
    } else {
        chromLength = (length + 1)/2 - 3 + 0; //subtract 2 to get chromosome length that is divided by 6
    }
...
//    System.out.println("----------------------------- next gen");
  ...
  public void crossover(Chromosome p1, Chromosome p2) {  //ordered crossover
    ...
    //fill in the rest of the bits from the opposite parent for child1
    for(int f = 0; f < loc1; f++) {
        child1.set(f, p2.get(f));
        child2.set(f, p1.get(f));
    }
    
    //fill in the rest of the bits from the opposite parent for child2
...
//    System.out.println("prime: " + fittestChrom.getPrime());
  ...
  private void print(Chromosome chrom) {
      ...
      System.out.print(" = " + chrom.getPrime() + " - eval -" + chrom.getEval());
      System.out.println("");
  }//print
